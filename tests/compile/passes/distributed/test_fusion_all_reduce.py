...
def test_rocm_aiter_all_reduce_rmsnorm_per_token_quant_fp8_fusion_pass_replace(
    ...
    * ``AiterAllreduceFusedAddRMSNormPerTokenQuantFP8Pattern`` (with-residual)
    ...
    if not AiterCustomAllreduce.build_supports_per_token_quant():
        pytest.skip(
            "aiter build is missing 'custom_fused_ar_rms_quant'; the per-token "
            "patterns aren't registered."
        )
...
def rocm_aiter_per_token_quant_fusion_pass_on_test_model(
    ...
    with set_current_vllm_config(vllm_config):
        ...
        # the unfused reference, so allow an FP8-scale tolerance (a wiring bug --
        # e.g. a scale/residual swap -- diverges far more than this).
